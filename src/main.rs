...
fn build_config(cli: &Cli) -> FilterConfig {
    FilterConfig {
        ...
        required_length: cli.length_required,
        max_length: cli.length_limit,
        quality_enabled: !cli.disable_quality_filtering,
        length_enabled: !cli.disable_length_filtering,
    }
}

fn pipeline(cli: &Cli) -> Result<FilterReport> {
    let cfg = build_config(cli);
    let p = Pipeline::new(&cfg, cli.compression);

    let report = match (cli.in2.as_ref(), cli.out2.as_ref()) {
        (Some(in2), Some(out2)) => p.run_pe(&cli.in1, in2, &cli.out1, out2)?,
        (None, None) => p.run_se(&cli.in1, &cli.out1)?,
        _ => {
